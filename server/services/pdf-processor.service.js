...
class PdfProcessorService {
  ...
  async processPdf(inputPath, outputPath, options) {
    try {
      ...
      this.addLog(`Error processing PDF: ${error.message}`);
      throw error;
    }
  }

  async extractAndAnalyzePages(inputPath, options) {
    ...
    for (let i = 0; i < totalPages; i++) {
      ...
      try {
        ...
        this.addLog(`Error processing page ${i + 1}: ${error.message}`);
  ...
  async analyzePage(page, pageIndex, options) {
    ...
    try {
      ...
      const imageHash = options.phash ? await this.generateImageHash(imageBuffer) : '';
      ...
      this.addLog(`Error analyzing page ${pageIndex + 1}: ${error.message}`);
      return {
        pageNumber: pageIndex,
        content: '',
        text: '',
  ...
  async generateImageHash(imageBuffer) {
    try {
      ...
      this.addLog(`Error generating image hash: ${error.message}`);
      return '';
    }
  }

  async detectPageRotation(imageBuffer) {
    try {
      ...
      this.addLog(`Error detecting page rotation: ${error.message}`);
      return 0;
    }
  }
  ...
  findNextBestPage(orderedPages, processed) {
  ...
  async reconstructPdf(inputPath, outputPath, orderedPages, toc) {
    ...
    try {
      ...
      for (const pageIndex of orderedPages) {
        if (pageIndex < this.pages.length) {
          ...
          if (this.pages[pageIndex].rotation !== 0) {
            copiedPage.setRotation({ angle: this.pages[pageIndex].rotation, type: 'degrees' });
          }
      ...
      if (toc.length > 0) {
        const tocPage = this.createTableOfContentsPage(toc);
      ...
      this.addLog(`Error reconstructing PDF: ${error.message}`);
      throw error;
    }
  }

  createTableOfContentsPage(toc) {
  ...
  generateReasoning(orderedPages, duplicates, toc) {
